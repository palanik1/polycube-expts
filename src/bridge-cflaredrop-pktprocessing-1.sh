...
function setup_expt {
    ...
    prefix="cf"
    add_modules "cflaredrop" ${prefix} ${j} XDP_SKB
    polycubectl ${prefix}1 ports add to_veth1 peer=veth1
    polycubectl ${prefix}1 ports add to_pp1
    
    # Add pkt prcessing modules
    prefix="pp"
    add_modules "pktprocessing" ${prefix} ${j} XDP_SKB
    polycubectl ${prefix}1 ports add to_cf1

    sleep 5
    
    polycubectl connect cf1:to_pp1 pp1:to_cf1
