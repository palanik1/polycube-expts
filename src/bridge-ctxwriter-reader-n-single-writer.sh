...
# Sets up experiment with ctxwriter1..ctxwritern
#Set up following topology
# veth1 <-> cw1_1 <-> cw2_1... <-> cwn+1_1 <-> br1  <-> veth2
function setup_expt_independant {
    ...
    add_simplebridges 1
    add_ctxwriters_independant ${j}

    polycubectl cw1_1 ports add to_veth1 peer=veth1
    polycubectl cw1_1 set action=WRITE
    ...
    connect_ctxwriters_independant $((${1}))
    k=$((${1}+1))
    for i in $(seq 2 ${k});
        do
	    set_action_ctxwriter_independant ${i} READ
	done

    #polycubegctl cw${j} set action=WRITE
    connect_ctxwriter_independant_with_bridge ${j}
